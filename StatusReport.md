...
Identify key regions for analysis. Initial data exploration and Data Lifecycle mapping: Completed. Identified 5+ major regions, implemented a data lifecycle tracking system, and conducted preliminary analysis by creating a ClimateEconomicDataPipeline class and creating a database schema.

Clean and create a master dataset: Completed. Integrated climate events with economic indicators by standardizing the format of names, dates, and values and removing empty values. Created a master dataset and filtered data to the relevant time period.

Normalize and assess quality of the dataset. Further break down data into cost, changes in frequency and severity, etc.: Completed. Standardized units and formats across the dataset, identified and resolved consistency issues within the dataset, and created a quality evaluation system. 
...
Create outline of findings and create report of findings (Both, not started)
